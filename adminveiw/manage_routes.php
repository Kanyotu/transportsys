...
if ($search != '') {
    $where_conditions[] = "(r.routename LIKE '%$search%' OR s.sacconame LIKE '%$search%')";
}

if ($filter_sacco) {
    $where_conditions[] = "r.saccoid = $filter_sacco";
}
            ...
            <div class="data-card" style="margin-bottom: 2rem; padding: 1.5rem 2rem;">
                <form method="GET" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.5rem; align-items: end;">
                    <div class="input-group" style="margin-bottom: 0;">
                        <label style="font-size: 0.75rem; font-weight: 600; color: var(--text-muted); margin-bottom: 0.5rem; display: block;">SEARCH ROUTES</label>
                        <div style="position: relative;">
                            <i class="fas fa-search" style="position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: var(--text-muted);"></i>
                            <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Route name..." style="padding-left: 35px; width: 100%;">
                        </div>
                    </div>
                    <div class="input-group" style="margin-bottom: 0;">
                        <label style="font-size: 0.75rem; font-weight: 600; color: var(--text-muted); margin-bottom: 0.5rem; display: block;">ASSIGNED SACCO</label>
                        <select name="sacco_id">
                            <option value="">All SACCOs</option>
                            <?php $saccos_list->data_seek(0); while($s = $saccos_list->fetch_assoc()): ?>
                    ...
                    <div style="display: flex; gap: 0.5rem; height: 42px;">
                        ...
                        <a href="manage_routes.php" class="btn" style="background: var(--border); color: var(--text-main); text-decoration: none; display: flex; align-items: center; justify-content: center; width: 42px;">
